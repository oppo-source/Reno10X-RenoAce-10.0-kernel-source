	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_FAST2NORMAL_CHG:
			if (oppo_vooc_get_fastchg_to_normal() == true
					|| oppo_vooc_get_fastchg_to_warm() == true
					|| oppo_vooc_get_btb_temp_over() == true
					|| oppo_vooc_get_fastchg_low_temp_full() == true) {
				val->intval = 1;
			} else {
				val->intval = 0;
			}
...
//#endif
		...
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			if (oppo_vooc_get_fastchg_started() == true && (chip->vbatt_num == 2)
...
static void oppo_chg_voter_charging_start(struct oppo_chg_chip *chip,
					OPPO_CHG_STOP_VOTER voter)
{
	chip->chging_on = true;
...
static void oppo_chg_voter_charging_stop(struct oppo_chg_chip *chip,
					OPPO_CHG_STOP_VOTER voter)
{
...
static void oppo_chg_check_cool_down_ichging(struct oppo_chg_chip *chip)
{
...
//	chip->anti_shake_bound.overtemp_bound = chip->limits.overtemp_bat_decidegc;
	...
	chip->short_c_batt.is_feature_sw_on
		= oppo_short_c_batt_feature_sw_status_init();
	chip->short_c_batt.is_feature_hw_on
		= oppo_short_c_batt_feature_hw_status_init();
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	if (ui_soc_cp_flag == 0) {
		if ((chip->soc < 0 || chip->soc > 100) && retry_counts < RETRY_COUNTS) {
			charger_xlog_printk(CHG_LOG_CRTI,
				"[Battery]oppo_chg_get_battery_data,\
...
static void oppo_chg_protection_check(struct oppo_chg_chip *chip)
{
	if (false == oppo_chg_check_tbatt_is_good(chip)) {
		...
		if ((chip->stop_voter & CHG_STOP_VOTER__BATTTEMP_ABNORMAL)
			...
			charger_xlog_printk(CHG_LOG_CRTI,
				"oppo_chg_check_tbatt_is_good func ,true! To Normal\n");
	...
	if (false == oppo_chg_check_vchg_is_good(chip)) {
		...
		if ((chip->stop_voter & CHG_STOP_VOTER__VCHG_ABNORMAL)
			...
			charger_xlog_printk(CHG_LOG_CRTI,
				"oppo_chg_check_vchg_is_good func ,true! To Normal\n");
	...
	if (chip->cool_down_done) {
		oppo_chg_check_cool_down_ichging(chip);
	}
}
...
static void oppo_chg_get_chargerid_voltage(struct oppo_chg_chip *chip)
{
	...
	if (oppo_vooc_get_vooc_switch_val() == 1) {
		...
		if (chip->chargerid_volt_got == false) {
			chip->chg_ops->set_chargerid_switch_val(1);
			oppo_vooc_set_vooc_chargerid_switch_val(1);
#ifdef CONFIG_OPPO_CHARGER_MTK
			usleep_range(100000, 110000);
#else
			usleep_range(20000, 22000);
			...
			chip->chargerid_volt = chip->chg_ops->get_chargerid_volt();
			chip->chargerid_volt_got = true;
			oppo_vooc_set_vooc_chargerid_switch_val(0);
...
static void oppo_chg_fast_switch_check(struct oppo_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (!oppo_vooc_get_fastchg_started()
				&& !oppo_vooc_get_fastchg_dummy_started()
...
static void oppo_chg_check_status_full(struct oppo_chg_chip *chip)
{
	...
	if (chip->chg_ctrl_by_vooc) {
		if (oppo_vooc_get_fastchg_ing()
				== true && oppo_vooc_get_fast_chg_type()
				!= CHARGER_SUBTYPE_FASTCHG_VOOC)
			return;
	} else {
		if (oppo_vooc_get_fastchg_ing() == true)
...
static void oppo_chg_kpoc_power_off_check(struct oppo_chg_chip *chip)
{
	...
	if (chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		...
		if ((chip->chg_ops->check_chrdet_status() == false)
			...
			if ((oppo_vooc_get_fastchg_to_normal() == false)
					&& (oppo_vooc_get_fastchg_to_warm() == false)
					&& (oppo_vooc_get_adapter_update_status() != ADAPTER_FW_NEED_UPDATE)
...
static void oppo_chg_pd_config(struct oppo_chg_chip *chip)
{
	...
	if (!chip->chg_ops->oppo_chg_pd_setup
			|| !chip->chg_ops->oppo_chg_get_pd_type) {
		return;
	}
...
int oppo_tbatt_power_off_task_init(struct oppo_chg_chip *chip)
{
	...
	chip->tbatt_pwroff_task
		= kthread_create(oppo_tbatt_power_off_kthread, chip, "tbatt_pwroff");
