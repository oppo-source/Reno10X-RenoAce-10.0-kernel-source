...
#include "oppo_battery_msm8150Q.h"
...
#include "../charger_ic/oppo_bq25882.h"
#include "../gauge_ic/oppo_bq27541.h"
#include "op_charge.h"

struct oppo_chg_chip *g_oppo_chip = NULL;
bool fg_oppo_set_input_current = false;
...
static void oppo_otg_disable_config(void);
static void oppo_set_otg_switch_status_default(bool value);
...
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/13, sjc Modifyfor charging */
...
#ifdef VENDOR_EDIT
	/* Jianchao.Shi@BSP.CHG.Basic, 2019/02/22, sjc Modify for QC */
	if (chip->vbatt_num == 2) {
				...
				union power_supply_propval *val)
{
	int rc;
#ifdef VENDOR_EDIT
/* Jianchao.Shi@PSW.BSP.CHG.Basic, 2018/04/19, sjc Modiy for CTS */
	struct oppo_chg_chip *chip = g_oppo_chip;
	if (chip) {
...
#ifdef VENDOR_EDIT
	/* tongfeng.Huang@PSW.BSP.CHG.Basic, 2019/05/16,  disable QC3 */
	mask = HVDCP_EN_BIT;
#else
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/19, sjc Add for charging */
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/03/28, sjc Modify for charging */
		...
		if (!(apsd_result->pst == POWER_SUPPLY_TYPE_USB_FLOAT &&
			...
			oppo_set_smb5_usb_props_type(apsd_result->pst);
		}
#endif
	}
...
#ifndef VENDOR_EDIT
	/*zhijie.li@BSP.CHG.Basic, 2019/1/9, lzj add for cancel the rerun_aicl*/
	/* Re-run AICL */
...
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/03/16, sjc Modify for qc */			
			val->intval = MICRO_12V;
#else
			val->intval = MICRO_9V;
#endif
	...
	case POWER_SUPPLY_TYPE_USB_PD:
#ifdef VENDOR_EDIT
/* zhijie.li@BSP.CHG.Basic, 2019/03/16, sjc Modify for pd */ 
		if (g_oppo_chip && g_oppo_chip->vbatt_num == 1)
...
#ifdef VENDOR_EDIT
/* tongfeng.Huang@BSP.CHG.Basic, 2019/01/10,  Add for c-c cable detect as water */
	msleep(5);
#endif
...
/* tongfeng.Huang@BSP.CHG.Basic, 2019/01/10,  Add for c-c cable detect as water */
	...
	if (chip && chip->vbatt_num == 2 && chip->chg_ops->get_charger_current){
		...
		pr_err("[OPPO_CHG], ibus current: val->intval[%d]", val->intval);
   ...
   if (chg->pd_active)  {
        ...
        if (rc < 0) {
            ...
            if (strcmp("UNKNOWN", apsd->name) == 0) {
                chg_err("start oppo_charger work\n");
		...
		oppo_force_to_fulldump(true);
			...
			if (chip->vbatt_num == 2) {
				if (g_oppo_chip && g_oppo_chip->ui_otg_switch != g_oppo_chip->otg_switch)
					oppo_set_otg_switch_status_default(g_oppo_chip->ui_otg_switch);
			}
		cancel_delayed_work_sync(&chg->chg_monitor_work);
	}
#endif
...
#endif
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/04/24, sjc Add to rerun apsd */
		chg->uusb_apsd_rerun_done = false;
#endif
...
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/04/24, sjc Modify to rerun apsd */
	if ((chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/12/13, sjc Add for usb water detect */
	smblib_err(chg, "%s: LPD_NONE detected\n", __func__);
	if ((oppo_get_usb_status() & USB_WATER_DETECT) != 0 && g_oppo_chip) {
...
static void typec_src_removal(struct smb_charger *chg)
{
	...
	#ifndef VENDOR_EDIT  /* zhangkun@BSP.CHG.Basic, 2019/02/22, Add for charge*/
	vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
			is_flash_active(chg) ? SDP_CURRENT_UA : SDP_100_MA);
	#else
	vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, false, 0);
...
irqreturn_t typec_or_rid_detection_change_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;
	#ifndef VENDOR_EDIT  /* zhangkun@BSP.CHG.Basic, 2019/02/22, Add for disable water detect*/
	u8 stat;
	int rc;
	#endif
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/30, sjc Add for using gpio as CC detect */
	...
	} else  if (oppo_ccdetect_support_check() == OPPO_NOT_SUPPORT_CCDETECT) {
		if (chg->typec_present == false) {
    			if(oppo_get_otg_switch_status() == false 
					&& oppo_ccdetect_get_power_role() != POWER_SUPPLY_TYPEC_PR_SINK) {
    				oppo_otg_disable_config();
    			}
		}
	}
#endif
		...
		if(oppo_ccdetect_check_is_gpio(chip) == true) { 
			...
			if (chg->pd_hard_reset != 0) {
				...
				if (rc < 0) {
					...
					printk(KERN_ERR "[OPPO_CHG][%s]: reg0x1368[0x%x]\n", __func__, typec_stat);
				}
				if(level != 1) {
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/30, sjc Add for using gpio as CC detect */
static void oppo_ccdetect_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						ccdetect_work.work);
	int level;
...
static void smblib_lpd_ra_open_work(struct work_struct *work)
{
	...
	if (smblib_rsbux_low(chg, RSBU_K_300K_UV)) {
		...
		chg->moisture_present =  true;
#ifdef VENDOR_EDIT
/* tongfeng.Huang@BSP.CHG.Basic, 2018/12/13, sjc Add for usb water detect */
		smblib_err(chg, "%s: LPD_MOISTURE_DETECTED detected\n", __func__);
		if ((oppo_get_usb_status() & USB_WATER_DETECT) == 0 && g_oppo_chip) {
                    oppo_vooc_set_disable_adapter_output(true);
			oppo_set_usb_status(USB_WATER_DETECT);
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/04/13, sjc Add for charging */
static void oppo_chargerid_switch_work(struct work_struct *work)
{
	smbchg_set_chargerid_switch_val(0);
	return;
}
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/30, sjc Add for using gpio as CC detect */
	INIT_DELAYED_WORK(&chg->ccdetect_work, oppo_ccdetect_work);
#endif
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2018/04/13, sjc Add for charging */
	INIT_DELAYED_WORK(&chg->typec_disable_cmd_work, typec_disable_cmd_work);
	INIT_WORK(&chg->chargerid_switch_work, oppo_chargerid_switch_work);
#endif
...
int oppo_usbtemp_adc_gpio1_init(struct oppo_chg_chip *chip)
{
	struct smb_charger *chg = NULL;
...
int oppo_usbtemp_adc_gpio12_init(struct oppo_chg_chip *chip)
{
	struct smb_charger *chg = NULL;
...
int oppo_charger_int_gpio_init(struct oppo_chg_chip *chip)
{
...
int oppo_input_pg_gpio_init(struct oppo_chg_chip *chip)
{
...
int oppo_charger_error_gpio_init(struct oppo_chg_chip *chip)
{
...
void oppo_ccdetect_disable(void)  //18115
{
	int rc;
	u8 stat;
	struct smb_charger *chg = NULL;
...
static void oppo_otg_disable_config(void) //no ccdetct 19081
{    
	int rc;
	u8 stat;
	struct smb_charger *chg = NULL;
...
static void oppo_get_usbtemp_volt(struct oppo_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chg->iio.usbtemp_v_chan)) {
		//printk(KERN_ERR "[OPPO_CHG][%s]: chg->iio.usbtemp_v_chan  is  NULL !\n", __func__);
...
usbtemp_next:
	...
	if (chip->vbatt_num == 2) {
		if (IS_ERR_OR_NULL(chg->iio.usbtemp_sup_v_chan)) {
			//printk(KERN_ERR "[OPPO_CHG][%s]: chg->iio.usbtemp_sup_v_chan  is  NULL !\n", __func__);
...
static int oppo_usbtemp_dischg_action(struct oppo_chg_chip *chip)
{
	int rc = 0;
	struct smb_charger *chg = NULL;
...
static int oppo_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (((chip->usb_temp_l >= USB_57C) && (chip->usbtemp_volt_l > USB_100C_VOLT))
			...
			count = 0;
			msleep(delay);
			wait_event_interruptible(oppo_usbtemp_wq, oppo_usb_or_otg_is_present() == true);
		}
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/01/30, sjc Add for using gpio as CC detect */
    ...
    oppo_usbtemp_adc_gpio1_init(chip);
    oppo_usbtemp_adc_gpio12_init(chip);
    oppo_charger_int_gpio_init(chip);
    oppo_input_pg_gpio_init(chip);
    oppo_charger_error_gpio_init(chip);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (((chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT) ||
		   (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB))
			&& ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB) || (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_CDP)))
			val->intval = 1;
		else
			val->intval = 0;
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	u8 val = 0;
    #endif
    struct oppo_chg_chip *oppo_chip = g_oppo_chip;
	union power_supply_propval pval;
...
static bool oppo_usb_or_otg_is_present(void)
{
	...
	if (chip->pmic_spmi.smb5_chip->chg.typec_mode == POWER_SUPPLY_TYPEC_SINK
		...
		chg_err("chg->typec_mode = SINK,oppo_usb_or_otg_is_present return true!\n");
		return true ;
	}
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/03/13, sjc Add for PD */
...
static bool oppo_hvdcp_is_enable(struct smb_charger *chg)
{
	int rc = 0;
	u8 stat = 0;
...
bool oppo_sm8150_get_pd_type(void)
{
	struct smb_charger *chg = NULL;
...
/* Jianchao.Shi@BSP.CHG.Basic, 2017/08/10, sjc Add for charging */
...
#ifdef VENDOR_EDIT
	/* zhijie.li@BSP.CHG.Basic, 2019/05/29, sjc Add for charging*/
	...
	if (rc >= 0) {
		...
		pr_err("[OPPO_CHG] test chg->iio.usb_supplementary_temp_adc\n");
	}
	pr_err("[OPPO_CHG] test chg->iio.usb_supplementary_temp_adc out here\n");
#endif
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/05/25, sjc Add for usbtemp */
	...
	if (rc >= 0) {
		...
		pr_err("[OPPO_CHG] test chg->iio.usb_temp_adc \n");
	}
	pr_err("[OPPO_CHG] test chg->iio.usb_temp_adc out here\n");
#endif
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	struct smb_charger *chg = &chip->chg;
#ifdef VENDOR_EDIT
/* tongfeng.Huang@BSP.CHG.Basic, 2019/02/23,  Add for shipmode*/
	int level = 0;
#endif
#ifdef VENDOR_EDIT
	/* Jianchao.Shi@BSP.CHG.Basic, 2017/01/22, sjc Add for charging*/
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/03/02, sjc Add for using gpio as shipmode stm6620 */
	...
	if (g_oppo_chip && g_oppo_chip->enable_shipmode && level != 1) {
		msleep(1000);
