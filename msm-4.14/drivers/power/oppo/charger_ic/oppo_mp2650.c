        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
        pr_err("%s:tongfeng reg[0x%02x]=0x%02x\n", __FUNCTION__, databuf[0], databuf[1]);
...
static int __mp2650_write_reg(int reg, int val)
{
	...
	if (reg_access_allow != 0) {
        ...
        chg_err("tongfeng test can not access registers\n");
		return 0;
	}
    ret = i2c_smbus_write_byte_data(chip->client, reg, val);
    if (ret < 0) {
...
extern 	bool p922x_wireless_charge_start(void);
...
int mp2650_set_mps_otg_voltage(void)
{
	int rc = 0;
    struct chip_mp2650 *chip = charger_ic;
...
{
    struct chip_mp2650 *chip = charger_ic;
    if(0){//p922x_chip && !p922x_chip->p922x_chg_status.charge_online) {
        chg_err("is not in wireless charge, return! \n");
        return true;
    }

    chg_err("init mp2650 hardware for wireless charge! \n");
...
int mp2650_hardware_init_for_wireless_charge(void)
{
    struct chip_mp2650 *chip = charger_ic;
    if(0){//p922x_chip && !p922x_chip->p922x_chg_status.charge_online) {
        chg_err("is not in wireless charge, return! \n");
    ...
    mp2650_input_current_limit_write(WPC_CHARGER_INPUT_CURRENT_LIMIT_DEFAULT);
...
struct oppo_chg_operations  mp2650_chg_ops = {
    ...
    .oppo_chg_get_pd_type = oppo_sm8150_get_pd_type,
    .enable_qc_detect = oppo_chg_enable_qc_detect,
