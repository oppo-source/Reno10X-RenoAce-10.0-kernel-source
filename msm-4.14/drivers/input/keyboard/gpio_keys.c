...
/*wanghao@BSP.Kernel.Debug, 2018/05/09, Add for triggering dump*/
	...
	/* Zhijun.ye@PSW.MM.Display.LCD.Stability, 2019/12/18,
	 * add for enabling 18503 project full dump */
	if (get_project() != OPPO_UNKOWN && (get_project() == OPPO_18503) && (door_open == false) && (1 == vol_Key_password)) {
		pr_err("get_project(%d), door_open:%d, vol_Key_password:%d, state:%d \n", get_project(), door_open, vol_Key_password, state);
		oppo_switch_fulldump(1);
		door_open = true;
	}
#endif
