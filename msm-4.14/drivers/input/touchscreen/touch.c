...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
    ...
    if (is_project(OPPO_18115) || is_project(OPPO_18501))
        prj_id = OPPO_18115;
    else if (is_project(OPPO_19081))
        prj_id = OPPO_19081;
    ...
    if (strstr(saved_command_line, "evt_panel") && is_project(OPPO_19081)) {
        snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
                "tp/%d/FW_%s_%s_OLD.img",
                prj_id, panel_data->chip_name, vendor);
        if (panel_data->test_limit_name) {
            snprintf(panel_data->test_limit_name, MAX_LIMIT_DATA_LENGTH,
                "tp/%d/LIMIT_%s_%s_OLD.img",
                prj_id, panel_data->chip_name, vendor);
        }
    } else {
        snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
