 ...
 * VENDOR_EDIT
 * File       : hx83112f_drivers_s4322.c
 * Description: Source file for hx83112f S4322 driver
 * Version   : 1.0
 * Date        : 2017-04-17
...
//    TPD_INFO("%s: CONFIG_VMAP_STACK = %d \n",
...
int hx_test_data_pop_out(struct chip_data_hx83112f *chip_info, char *g_Test_list_log, char *g_Company_info_log, char *g_project_test_info_log, char *rslt_buf, char *filepath)
{
        ...
        if(isBD12proj || isEA006proj) {
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if(isRead_csv == false) {
                    ...
                    if (RAW[i] > hx83112f_nf_inspection_criteria[IDX_LPWUG_NOISE_MAX][i] ||
                        RAW[i] < hx83112f_nf_inspection_criteria[IDX_LPWUG_NOISE_MIN][i]) {
                        TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n", __func__);
                        ret = THP_AFE_INSPECT_ENOISE;
                    }
                }
    ...
    case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                ...
                if (RAW[i] > hx83112f_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MAX][i] ||
                    RAW[i] < hx83112f_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MIN][i]) {
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE FAIL\n", __func__);
                    ret = THP_AFE_INSPECT_ENOISE;
                }
            }
    ...
    for (i = 0; i < HX_CRITERIA_SIZE; i++) {
        ...
        if (hx83112f_nf_inspt_crtra_flag[i] == 1) {
            for (j = 0; j < all_mut_len; j++) {
                TPD_DEBUG("%d, ", hx83112f_nf_inspection_criteria[i][j]);
                if (j % 16 == 15)
                    TPD_DEBUG("\n");
            }
        } else {
...
uint8_t himax_read_DD_status(struct chip_data_hx83112f *chip_info, uint8_t *cmd_set, uint8_t *tmp_data)
{
    int cnt = 0;
...
static size_t hx83112f_proc_DD_debug_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    int ret = 0;
    uint8_t tmp_data[64];
    uint8_t loop_i = 0;
...
static size_t hx83112f_proc_DD_debug_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    uint8_t i = 0;
    uint8_t cnt = 2;
    unsigned long result = 0;
...
int himax_read_FW_status(struct chip_data_hx83112f *chip_info, uint8_t *state_addr, uint8_t *tmp_addr)
{
    uint8_t req_size = 0;
...
static size_t hx83112f_proc_FW_debug_read(struct file *file, char *buf,
...
static int hx83112f_reset(void *chip_data)
{
    ...
    /*Yulianghan@RM.PSW.BSP.TP, 2018/09/07, add for hx83112f_noflash lcd esd TP irq exception in realme 18612*/
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
    enable_irq(chip_info->hx_irq);
#endif
    //hx83112f_enable_interrupt(g_chip_info, true);
...
static int hx83112f_enable_black_gesture(struct chip_data_hx83112f *chip_info, bool enable)
{
    ...
    if (ts->is_suspended) {
        if (enable) {
            ...
            //himax_sense_on(0);
            /*if (!HX_RESET_STATE) {
                ret =  hx83112f_resetgpio_set(chip_info->hw_res, true); // reset gpio
                if (ret < 0) {
                    TPD_INFO("%s: hx83112f reset gpio failed.\n", __func__);
                    return ret;
                }
            }*/
            /*ret = hx83112f_enable_interrupt(chip_info, true);
            ...
            /*if (HX_RESET_STATE) {
                ret =  hx83112f_resetgpio_set(chip_info->hw_res, false); // reset gpio
                if (ret < 0) {
                    TPD_INFO("%s: hx83112f reset gpio failed.\n", __func__);
                    return ret;
                }
...
/*
static int hx83112f_enable_edge_limit(struct chip_data_hx83112f *chip_info, bool enable)
{
    int ret = 0;
    return ret;
}
...
static void hx83112f_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    ...
    if (error_count) {
        ...
        hx_test_data_pop_out(chip_info, g_Test_list_log, g_Company_info_log, g_project_test_info_log, g_rslt_data, g_file_path_NG);
        TPD_INFO("%s: g_file_path_NG = %s \n", __func__, g_file_path_NG);
    } else {
        snprintf(g_file_path_OK,
                 (int)(strlen(HX_RSLT_OUT_PATH_OK) + strlen(g_file_name_OK) + 1),
                 "%s%s", HX_RSLT_OUT_PATH_OK, g_file_name_OK);
        hx_test_data_pop_out(chip_info, g_Test_list_log, g_Company_info_log, g_project_test_info_log, g_rslt_data, g_file_path_OK);
        TPD_INFO("%s: g_file_path_OK = %s \n", __func__, g_file_path_OK);
    }
    /*Save Log Data */
...
static struct himax_proc_operations hx83112f_proc_ops = {
    ...
    .himax_proc_DD_debug_read =  hx83112f_proc_DD_debug_read,
    .himax_proc_DD_debug_write =  hx83112f_proc_DD_debug_write,
    .himax_proc_FW_debug_read =  hx83112f_proc_FW_debug_read,
...
/*Himax_DB_Test Start*/
#if 0
    } else if (is_oem_unlocked()) {//is_oem_unlocked()
#else
    } else if (1) {
#endif
/*Himax_DB_Test End*/
