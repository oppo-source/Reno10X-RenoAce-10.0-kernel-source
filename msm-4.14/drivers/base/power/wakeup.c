...
static char *irq_name_powerkey = "pon_kpdpwr_status";
static char *irq_name_rtc_alarm = "qpnp_rtc_alarm";
...
void pm_system_irq_wakeup(unsigned int irq_number)
{
	...
	if (pm_wakeup_irq == 0) {
		if (msm_show_resume_irq_mask) {
            ...
            #ifdef VENDOR_EDIT
            //SunFaliang@BSP.Power.Basic, 2019/10/25, add for analysis power coumption.
			if (strcmp(name, irq_name_powerkey) == 0) {
				wakeup_source_count_kpdpwr++;
			} else if(strcmp(name, irq_name_rtc_alarm) == 0) {
