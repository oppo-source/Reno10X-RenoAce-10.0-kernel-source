...
static int pm8008_regulator_enable(struct regulator_dev *rdev)
{
	...
	pm8008_debug(pm8008_reg," pm8008-ldo init_mv = %d",init_mv);

	/* Get fingherprint regulator for pm8008 reset*/
	if( init_mv > 3200 && g_fingherprint_rdev == NULL ) {
		g_fingherprint_rdev = rdev ;
		pm8008_err(pm8008_reg,"pm8008 get g_fingherprint_rdev init_mv = %d",init_mv);
	}
#endif
	if (init_mv < 0) {
		pm8008_err(pm8008_reg,
