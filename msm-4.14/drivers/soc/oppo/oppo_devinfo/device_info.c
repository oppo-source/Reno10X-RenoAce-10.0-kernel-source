...
struct devinfo_data {
    ...
    struct pinctrl_state *hw_sub1_active;
	struct pinctrl_state *hw_sub1_sleep;
    struct pinctrl_state *hw_sub2_active;
	struct pinctrl_state *hw_sub2_sleep;
...
static void sub_mainboard_verify(struct devinfo_data *devinfo_data)
{
        ...
        devinfo_data->hw_sub1_active = pinctrl_lookup_state(devinfo_data->pinctrl, "hw_sub1_active");
        if (IS_ERR_OR_NULL(devinfo_data->hw_sub1_active)) {
                pr_err("%s:%d Failed to get the gpio8 active state pinctrl handle\n", __func__, __LINE__);
        }
        devinfo_data->hw_sub1_sleep = pinctrl_lookup_state(devinfo_data->pinctrl, "hw_sub1_sleep");
        if (IS_ERR_OR_NULL(devinfo_data->hw_sub1_sleep)) {
                pr_err("%s:%d Failed to get the gpio8 suspend state pinctrl handle\n", __func__, __LINE__);
        }

        devinfo_data->hw_sub2_active = pinctrl_lookup_state(devinfo_data->pinctrl, "hw_sub2_active");
        if (IS_ERR_OR_NULL(devinfo_data->hw_sub2_active)) {
                pr_err("%s:%d Failed to get the gpio2 active state pinctrl handle\n", __func__, __LINE__);
        }
        devinfo_data->hw_sub2_sleep = pinctrl_lookup_state(devinfo_data->pinctrl, "hw_sub2_sleep");
        if (IS_ERR_OR_NULL(devinfo_data->hw_sub2_sleep)) {
                pr_err("%s:%d Failed to get the gpio2 suspend state pinctrl handle\n", __func__, __LINE__);
        }

        if (!IS_ERR_OR_NULL(devinfo_data->hw_sub1_active)) {
                pinctrl_select_state(devinfo_data->pinctrl, devinfo_data->hw_sub1_active);
        }
        if (!IS_ERR_OR_NULL(devinfo_data->hw_sub2_active)) {
                pinctrl_select_state(devinfo_data->pinctrl, devinfo_data->hw_sub2_active);
        }
...
sub_mainboard_set:
        ...
        if (!IS_ERR_OR_NULL(devinfo_data->hw_sub1_sleep)) {
                pinctrl_select_state(devinfo_data->pinctrl, devinfo_data->hw_sub1_sleep);
        }
        if (!IS_ERR_OR_NULL(devinfo_data->hw_sub2_sleep)) {
                pinctrl_select_state(devinfo_data->pinctrl, devinfo_data->hw_sub2_sleep);
        }
