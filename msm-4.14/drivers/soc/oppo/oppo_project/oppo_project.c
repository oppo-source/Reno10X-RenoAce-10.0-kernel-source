...
void init_project_version(void)
{
	...
	if (format == ERR_PTR(-EPROBE_DEFER)) {
		...
		pr_err("engVersion=%d\n", format->nEngVersion);
	pr_err("ProjectInfoCDTType=%d, smem_size=%d\n", sizeof(ProjectInfoCDTType),smem_size);
}
#endif//VENDOR_EDIT
...
bool is_confidential(void)
{
	...
	return format->nEngVerConfidential;
}
...
bool oppo_daily_build(void)
{
	...
	if ((ALL_NET_CMCC_TEST == format->nEngVersion) || (ALL_NET_CMCC_FIELD == format->nEngVersion) ||
		(ALL_NET_CU_TEST == format->nEngVersion) || (ALL_NET_CU_FIELD == format->nEngVersion) ||
		(ALL_NET_CT_TEST == format->nEngVersion) || (ALL_NET_CT_FIELD == format->nEngVersion))
		return false;
				...
				size_t count, loff_t *off)
{
	...
	snprintf(page, 63, "%d", get_eng_version());
...
static ssize_t machinetype_read_proc(struct file *file, char __user *buf,
				size_t count, loff_t *off)
{
	char page[256] = {0};
	int len = 0;

	len = sprintf(page, "%d", get_Machine_Type());
...
struct file_operations machinetype_proc_fops = {
		.read = machinetype_read_proc,
};
...
/*Ziqing.Guo@BSP.Fingerprint.Secure 2017/04/16 Add for distinguishing secureboot stage*/
	...
	pentry = proc_create("machinetype", S_IRUGO, oppoVersion, &machinetype_proc_fops);
	if (!pentry) {
		pr_err("create machinetype proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
