...
static const struct hypnus_ioctl hypnus_ioctl_funcs[] = {
	...
	HYPNUS_IOCTL_FUNC(IOCTL_HYPNUS_SET_LPM_GOV,
	...
	/* NPU */
	HYPNUS_IOCTL_FUNC(IOCTL_HYPNUS_SUBMIT_NPUFREQ,
				hypnus_ioctl_submit_npufreq),
	/* misc */
	HYPNUS_IOCTL_FUNC(IOCTL_HYPNUS_POLL_STATUS,
				hypnus_ioctl_poll_status),
	HYPNUS_IOCTL_FUNC(IOCTL_HYPNUS_GET_SOC_INFO,
	...
	/* DDR */
	HYPNUS_IOCTL_FUNC(IOCTL_HYPNUS_SUBMIT_DDR,
				hypnus_ioctl_submit_ddr),
};
...
static int hypnus_open(struct inode *inode, struct file *fp)
{
	...
	pdata = container_of(inode->i_cdev, struct hypnus_data, cdev);
	...
	unsigned long arg)
{
	...
	ret = hypnus_ioctl_funcs[nr].func(pdata, cmd, data);
	if (ret)
		return ret;
...
int hypnus_dev_init(struct hypnus_data *pdata)
{
	int ret = 0;
	struct device *dev;
	...
	mutex_init(&pdata->cdev_mutex);

	ret = alloc_chrdev_region(&pdata->dev_no, 0, 1, "hypnus");
	if (ret) {
		pr_err("Fail to alloc devid, ret = %d\n", ret);
		goto err_cdev_alloc;
	}

	cdev_init(&pdata->cdev, &hypnus_fops);

	ret = cdev_add(&pdata->cdev, pdata->dev_no, 1);
	if (ret) {
		pr_err("Fail to add cdev\n");
		goto err_cdev_add;
	}

	pdata->class = class_create(THIS_MODULE, "hypnus");
	if (!pdata->class || IS_ERR(pdata->class)) {
		pr_err("Fail to add hypnus class\n");
		goto err_class_add;
	}

	dev = device_create(pdata->class, NULL, pdata->dev_no,
				NULL, "hypnus");
	if (!dev || IS_ERR(dev)) {
...
int hypnus_dev_uninit(struct hypnus_data *pdata)
{
	device_destroy(pdata->class, pdata->dev_no);
	class_destroy(pdata->class);
	cdev_del(&pdata->cdev);
	unregister_chrdev_region(pdata->dev_no, 1);
	platform_driver_unregister(&hypnus_pdrv);
