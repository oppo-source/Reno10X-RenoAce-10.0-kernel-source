...
static uint8_t AW8697_HAPTIC_RAM_VBAT_COMP_GAIN = 0x80;
...
static char aw8697_old_steady_test_rtp_name[5][30] ={
    {"aw8697_old_steady_168.bin"},
    {"aw8697_old_steady_170.bin"},
    {"aw8697_old_steady_173.bin"},
    {"aw8697_old_steady_175.bin"},
};
...
/* tongfeng.Huang@BSP.CHG.Basic, 2018/11/17,  Add for config trig1 */
    ...
    {"barca_alarm_ring_RTP_120.bin"},
    {"barca_incoming_ring_RTP_121.bin"},
    {"barca_notice_ring_RTP_122.bin"},
...
static char aw8697_ram_name_19081[5][30] ={
    {"aw8697_haptic_19081.bin"},
    {"aw8697_haptic_19081.bin"},
    {"aw8697_haptic_19081.bin"},
    {"aw8697_haptic_19081.bin"},
    {"aw8697_haptic_19081.bin"},
...
static int aw8697_ram_update(struct aw8697 *aw8697)
{
    ...
    if (prj_id == 19081) {
        pr_info("%s:19081 haptic bin name  %s \n", __func__,aw8697_ram_name_19081[index]);
        return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
                    aw8697_ram_name_19081[index], aw8697->dev, GFP_KERNEL,
...
const struct firmware *aw8697_rtp_load_file_accord_f0(struct aw8697 *aw8697)
{
    ...
    if (aw8697->rtp_file_num == AW8697_OPPO_WAVEFORM_INDEX_OLD_STEADY)
    {
        ...
        if (aw8697->rtp_file_num == AW8697_OPPO_RTP_LONG_SOUND_INDEX) {
            ...
            ret = request_firmware(&rtp_file,
                    aw8697_old_steady_test_rtp_name[f0_file_num],
                    aw8697->dev);
        }
        if(ret < 0)
        {
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
    ...
    if (!rtp_file)
    {
        ...
        if (prj_id != 19081) {
            ...
            ret = request_firmware(&rtp_file,
            aw8697_rtp_name_19081[aw8697->rtp_file_num],
            aw8697->dev);
        }
        if(ret < 0)
        {
    ...
    if (prj_id != 19081) {
        ...
        pr_info("%s: rtp file [%s] size = %d\n", __func__,
            aw8697_rtp_name_19081[aw8697->rtp_file_num], aw8697_rtp->len);
    }
        ...
        const char *buf, size_t count)
{
    ...
    aw8697_interrupt_clear(aw8697);
    mutex_unlock(&aw8697->lock);
    if(val < (sizeof(aw8697_rtp_name_19081)/AW8697_RTP_NAME_MAX)) {
