...
void oppo_parse_motor_info(struct oppo_motor_chip * chip)
{
	...
	if (is_project(OPPO_18115)) {
		if ((pcb_version <= HW_VERSION__13) || (HW_VERSION__16 == pcb_version)) {   //DVT before and DVT2
	...
	} else if (is_project(OPPO_18119)) {
		chip->pwm_param.normal.up_brake_delay = 40000;
		chip->pwm_param.normal.down_brake_delay = 25000;
		chip->stop.neg[1] = -2;
		chip->stop.pos[1] = 2;
	} else if (is_project(OPPO_18117)) {
		chip->pwm_param.normal.up_brake_delay = 40000;
		chip->pwm_param.normal.down_brake_delay = 25000;
		chip->stop.neg[1] = -2;
		chip->stop.pos[1] = 2;
	...
	MOTOR_ERR("project(%d), pcb_version(%d), up_down(%d, %d).\n",
		get_project(), pcb_version, chip->pwm_param.normal.up_brake_delay, chip->pwm_param.normal.down_brake_delay);
...
void oppo_m1120_reconfig(struct oppo_dhall_chip *chip)
{
	...
	MOTOR_ERR("project(%d), pcb_version(%d), range(%d).\n", get_project(), get_PCB_Version(), chip->reg.map.range);
}
